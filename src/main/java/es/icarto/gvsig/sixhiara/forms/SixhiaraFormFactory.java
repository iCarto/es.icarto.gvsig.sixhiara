...
public class SixhiaraFormFactory extends DBConnectionBaseFormFactory {
	...
	public AbstractSubForm createSubForm(String tableName) {
		if (tableName != null) {
			...
			} else if (tableName.equals(QuantidadeAguaSubForm.TABLENAME)) {
				return new QuantidadeAguaSubForm();
			} else if (tableName.equals(DadosPluviometricosSubForm.TABLENAME)) {
				return new DadosPluviometricosSubForm();
			} else if (tableName.equals(DadosHidrometricosSubForm.TABLENAME)) {
				return new DadosHidrometricosSubForm();
			} else if (tableName.equals(FontesLitologiaSubForm.TABLENAME)) {
				return new FontesLitologiaSubForm();
			} else if (tableName.equals(FontesCaracHidroSubForm.TABLENAME)) {
	...
	public void loadTable(String tableName) {
		/*
		 * As nt formfactory only allows register one factory we should do this ugly if
		 * instead of have a factory for each project
		 */
